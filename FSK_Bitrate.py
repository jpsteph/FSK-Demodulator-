...
def init_radio(Fs, frf, gain = None):
    sdr = RtlSdr()
    sdr.sample_rate = Fs
    sdr.center_freq = frf
    if gain != None:     
...
def main(Fs, #sampling rate of sdr
    ...
    #position of 9600 Hz peak will be (fftsize/2)*bps/(samplerate/2)
    if debug:
...
if __name__ == "__main__":
    ...
    bps = 28800, 
    fromIQ = True, 
    IQpath = r"C:\Users\jstephenson\Documents\IQ RECORDINGS\LFMFSK28800", 
    IQindex = list(range(390000,420000)))